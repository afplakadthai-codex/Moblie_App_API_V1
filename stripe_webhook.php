...
// ── Emergency logging (writes before any require_once, before any other code) ──
function bv_webhook_emergency_log(string $message, array $context = []): void
{
    $logDir = __DIR__ . '/logs';
    if (!is_dir($logDir)) {
    ...
    @file_put_contents($logDir . '/stripe_webhook_emergency.log', $entry . PHP_EOL, FILE_APPEND | LOCK_EX);
}

bv_webhook_emergency_log('webhook_entered', [
    'request_method'           => $_SERVER['REQUEST_METHOD'] ?? 'unknown',
    'content_length'           => $_SERVER['CONTENT_LENGTH'] ?? $_SERVER['HTTP_CONTENT_LENGTH'] ?? 0,
    'stripe_signature_present' => isset($_SERVER['HTTP_STRIPE_SIGNATURE']) && $_SERVER['HTTP_STRIPE_SIGNATURE'] !== '' ? 'yes' : 'no',
]);
...
function bv_sw_save_webhook_event(PDO $pdo, array $payload): void
{
    ...
    // If status column exists and event_id is set, check for an existing
    // unprocessed row and UPDATE it instead of INSERT to avoid UNIQUE(event_id) conflicts.
    $eventId = $data['event_id'] ?? '';
    if ($eventId !== '' && isset($cols['status'])) {
        $checkStmt = $pdo->prepare(
            'SELECT id, status FROM webhook_events WHERE event_id = ? LIMIT 1'
        );
        ...
        if ($existing !== false) {
            if ((string)$existing['status'] === 'processed') {
                // Already successfully processed — leave the row untouched.
                return;
            }

            // Existing row is unprocessed (failed/ignored/partial) — update it
            // with the latest status and payload so the retry is recorded cleanly.
            $setSql    = [];
            $setParams = [];
            $updateable = ['event_type', 'object_id', 'status', 'payload_json', 'processed_at'];
            foreach ($updateable as $k) {
                if (isset($cols[$k])) {
                    $setSql[]    = "`{$k}` = ?";
                    $setParams[] = $data[$k];
                }
            ...
            if ($setSql) {
                ...
                $pdo->prepare(
                    'UPDATE webhook_events SET ' . implode(', ', $setSql) . ' WHERE id = ? LIMIT 1'
                )->execute($setParams);
                bv_sw_log('webhook_event_row_updated_for_retry', [
                    'event_id'     => $eventId,
...
function bv_sw_has_processed_event(PDO $pdo, string $eventId): bool
{
    ...
    try {
        if (bv_sw_has_col($pdo, 'webhook_events', 'status')) {
            $stmt = $pdo->prepare('SELECT id FROM webhook_events WHERE event_id = ? AND status = ? LIMIT 1');
            $stmt->execute([$eventId, 'processed']);
        } else {
...
if ($eventId !== '' && bv_sw_has_processed_event($pdo, $eventId)) {
    bv_sw_log('duplicate_event_skip_processed_only', ['event_id' => $eventId, 'event_type' => $eventType]);
...
try {
    ...
    // Commit order-update transaction before calling paid handler —
    // order_paid_handler opens its own transaction and will error if one is already active.
    $pdo->commit();
    bv_sw_debug('tx_committed_before_paid_handler', ['order_id' => $orderId]);

    // Safety guard: must not enter paid handler with an active transaction.
    if ($pdo->inTransaction()) {
        bv_sw_log('paid_handler_blocked_active_transaction', [
            'event_id' => $eventId,
            'order_id' => $orderId,
        ]);
        $pdo->rollBack();
        bv_sw_json_response(500, ['ok' => false, 'error' => 'active_transaction_before_paid_handler']);
    }

    bv_sw_log('paid_handler_call_after_webhook_commit', [
        'event_id' => $eventId,
        'order_id' => $orderId,
    ]);
    ...
    // Save webhook_events AFTER paid handler — idempotency: only mark 'processed' when handler succeeded.
    bv_sw_save_webhook_event($pdo, [
        'event_id'     => $eventId,
        'event_type'   => $eventType,
        'object_id'    => $objectId ?: null,
    ...
    if (empty($result['ok'])) {
        // business failure: stop Stripe retry, return 200
...
} // end inner catch (transaction block)

} catch (Throwable $e) {
    // Outer catch: fires for exceptions BEFORE the inner transaction try/catch
    // (e.g. bootstrap failures, require_once errors, PDO unavailable, signature errors)
    if (isset($pdo) && $pdo instanceof PDO && $pdo->inTransaction()) {
        $pdo->rollBack();
    }

    bv_webhook_emergency_log('webhook_exception', [
